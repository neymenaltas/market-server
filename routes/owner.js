...
router.get("/report", verifyToken, restrictTo("owner"), async (req, res) => {
  ...
  router.patch(
    ...
    async (req, res) => {
      try {
        ...
        place.crashMessage = crashMessage || "";
        await place.save();

        // IO artık parametre olarak geldi, direkt kullanabiliriz
        io.to(placeId).emit("crash-message-updated", {
          placeId: placeId,
          crashMessage: place.crashMessage,
          updatedAt: new Date(),
        });

        res.status(200).json({
          success: true,
          message: "Crash message başarıyla güncellendi",
          crashMessage: place.crashMessage,
        });
      } catch (error) {
        console.error("Crash message güncelleme hatası:", error);
        res.status(500).json({
          success: false,
    ...
    async (req, res) => {
      try {
        ...
        if (existingUser) {
          return res
            .status(400)
            .json({ message: "Bu username ile zaten kayıt olunmuş." });
        }
    ...
    async (req, res) => {
      try {
        ...
        if (!place) {
          return res
            .status(404)
            .json({ message: "Geçerli bir mekan bulunamadı." });
        }
        ...
        if (place.ownerId.toString() !== req.user.id) {
          return res
            .status(403)
            .json({ message: "Bu mekana ürün ekleme yetkiniz yok." });
        }
        ...
        if (error.code === 11000) {
          return res
            .status(400)
            .json({ message: "Bu mekanda bu ürün zaten mevcut." });
        }
        res
          .status(500)
          .json({ message: "Sunucu hatası", error: error.message });
    ...
    async (req, res) => {
      try {
        ...
        const products = await Product.find({
          ...
          "productName regularPrice currentPrice previousPrice minPrice maxPrice placeId"
        );
    ...
    async (req, res) => {
      try {
        ...
        if (!worker) {
          return res
            .status(404)
            .json({ message: "Silinmek istenen worker bulunamadı." });
        }
        ...
        if (worker.role !== "worker") {
          return res
            .status(400)
            .json({ message: "Bu kullanıcı bir worker değil." });
        }
        ...
        if (!ortakPlace) {
          return res
            .status(403)
            .json({
              message: "Bu worker sizin mekanlarınıza ait değil, silemezsiniz.",
            });
        }
    ...
    async (req, res) => {
      try {
        ...
        if (!product) {
          return res
            .status(404)
            .json({ message: "Silinmek istenen product bulunamadı." });
        }
      ...
      try {
        ...
        if (username !== user.username) {
          ...
          if (existingUser) {
            return res
              .status(400)
              .json({ message: "Bu username zaten kullanılıyor." });
          }
        }
  ...
  router.get("/crash-message/:placeId", verifyToken, async (req, res) => {
    try {
      ...
      const place = await Place.findById(placeId).select("crashMessage");
      ...
      console.error("Crash message getirme hatası:", error);
      res.status(500).json({
        success: false,
        message: "Sunucu hatası",
        error: error.message,
